...
// Extending the default Form to avoid display flickering.  // Janro
...
let show (t: string) (bmp: bitmap) : unit =
  let h = bmp.Height
  let w = bmp.Width
  let form = new SmoothForm (Visible=true, Text=t, Height=h, Width=w, TopMost=true)  // Janro
...
let runApp (t: string) (w: int) (h: int)
  ...
  let form = new SmoothForm (Visible=true, Text=t, Width=w, Height=h, TopMost=true)  // Janro
  let state = ref s
