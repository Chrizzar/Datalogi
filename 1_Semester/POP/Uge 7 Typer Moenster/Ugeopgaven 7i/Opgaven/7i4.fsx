...
/// <summary> The function leven_cached will with two mutually inner recursive functions leven_cache and leven, return the difference in edits between two words. As well as how many times we hit a calculation we already have calculated, and how many calculations it took to calculate the result. </summary>
...
/// <returns> It returns the difference in edits between two words. As well as how many times we hit a calculation we already have calculated, and how many calculations it took to calculate the result. </returns>
...
let leven_cashed (a:string) (b:string) (i:int) (j:int): int =                // Definition
    ...
    // let Array_init n = Array.init (n) (fun x -> -1)
    let cache = Array.init (a.Length) (fun x -> Array.init (b.Length) (fun x -> -1))
    // let cache = Array.init (a.Length) (Array_init b.Length)


    /// <summary> The inner mutually recursive functions leven_cache and leven, can call each other. Leven_cache will do lookups in the cache, by checking if we don't have a calculation and then insert it in the cache. If we already have a calculation in the cache, we will get a cache hit, and therefore the program don't have to calculate it again. </summary>
    /// <remark> In the function leven_cache, where we inserts a calculation in the cache, we call the other recursive function leven, which we use for the calculation work. (We make the two mutually recursive functions, so they can call each other). <remarks>
    /// <param name="a"> The character string a </param>
    /// <param name="b"> The charecter string b </param>
    /// <param name="i"> The length of a </param>
    /// <param name="j"> The length of b </param>
