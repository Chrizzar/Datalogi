...
type king(col : Color) =
  ...
  override this.availableMoves (board : Board) : (Position list * chessPiece list) =
    let mutable tempMoves = board.getVacantNNeighbours this
    let mutable enemyMoves = []
    ...
    for i=0 to (Collections.Array2D.length1 board.BoardArray)-1 do
        for j=0 to (Collections.Array2D.length2 board.BoardArray)-1 do
            ...
            | Some p when p.color <> this.color && p.nameOfType <> "king" ->
                enemyMoves <- enemyMoves @ (fst (p.availableMoves board))
            | _ -> 
                ()

    let allEnemyMoves = Set.ofList enemyMoves
    let kingTempMoves =  Set.ofList (fst(tempMoves))

    ((Set.toList (kingTempMoves - allEnemyMoves), snd(tempMoves)))
