...
/// Using simple turtle graphics to produce a list of points on a polygon.
/// The view and connection parts are identical to fil 23.7_transformWindows.fsx

// Open often used libraries, be ware of namespave polution
...
// Turtle commands, type definition mus be in outer most scope
type Command = F | L | R
...
let model () : Size * ((Pen * (Point [])) list) =
    // Hilber recursion production rules
    ...
    let addRev (lst : Turtle list) (cmd : Command) (len : float) : Turtle
        list =
            let toInt = int << round
            match lst with
                | t::rest ->
                    match cmd with
                        ...
                        | F -> {t with x = t.x + len * cos t.d; y = t.y + len * sin t.d}::lst // Forward
    ...
    let len = 20.0
    let line =
        // Convert command list ot reverse turtle list
        ...
        |> List.toArray
    let black = new Pen (Color.FromArgb (50, 255, 50))
