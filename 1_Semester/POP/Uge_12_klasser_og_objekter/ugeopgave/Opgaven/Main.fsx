type Deck() = class
    ...
    let mutable _deck =
        // Card represented as tuples of (card_value (int), card_terminal_representation (string))
...
type Dealer(playerAmount: int, playingDeck: Deck) = class
    ...
    do printfn "Dealer is ready!"
    let mutable _handList = []
    let mutable _handSum = 0
    let mutable _playing = true
end

type Game(playerAmount: int, playingDeck: Deck, gameDealer: Dealer) = class
    ...
    member this.StartGame() =
            for player in this.PlayerList() do
                while player.GetPlaying() do
                ...
                | "stand" -> player.Stand()
                | "Stand" -> player.Stand()
                | _ -> failwith ("You can only choose between hitting or standing")

    new(playingDeck: Deck, gameDealer: Dealer) = 
        let random = System.Random()
        let amountOfPlayers = random.Next(1,6)
        Game(amountOfPlayers, playingDeck, gameDealer)
end

let main() = 
    let gameDeck = new Deck()
    let gameDealer = new Dealer(1, gameDeck)
    let playingGame = new Game(gameDeck, gameDealer)
